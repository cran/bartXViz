#' @source https://github.com/kapelner/bartMachine/tree/master/bartMachine
#' 
...
pre_process_new_data <- function(new_data, bart_machine){
	...
	if (bart_machine$impute_missingness_with_rf_impute){
		...
		imputations <- missForest::missForest(rbind(new_data, bart_machine$X), verbose = bart_machine$verbose)$ximp
